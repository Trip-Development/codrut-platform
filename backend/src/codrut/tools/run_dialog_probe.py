...
async def run_dialog_probe() -> None:
    ...
    # 1. Asigurare seed complet pentru temă, pachet, scenariu și setări
    ...
    async with SessionLocal() as db:
        ...
        if not project:
            stmt_proj = select(CompanyProject).limit(1)
            project = (await db.execute(stmt_proj)).scalar_one()

        # 2. Utilizator & profil participant
        ...
        # 3. Asigurare setări active cu cota de 20 replici
        ...
        if prog_settings:
            ...
            prog_settings.max_turns_per_session = 20
