...
@pytest.mark.asyncio
async def test_guest_account_session_cannot_open_participant_workspace() -> None:
    principal = SessionPrincipal(
        user_id=uuid.uuid4(),
        email="temporary@example.com",
        role=UserRole.participant,
        account_type=UserAccountType.guest,
        available_workspaces=(UserRole.participant,),
        default_workspace=UserRole.participant,
        terms_accepted_at=datetime.now(UTC),
        terms_version=CURRENT_TERMS_VERSION,
        session_token="guest-account-session",  # noqa: S106
        access_mode="account",
    )
    ...
    assert exc_info.value.code == "guest_dashboard_forbidden"
