...
async def test_practice_session_lifecycle_and_prompt_version():
    ...
    assert session_data["prompt_version"] == "v2.8-fara-filozofie"
    ...
    assert hist_data["session"]["prompt_version"] == "v2.8-fara-filozofie"
...
async def test_trainer_direct_entry_succeeds_when_enabled():
    ...
    data = resp.json()
    assert data["prompt_version"] == "v2.8-fara-filozofie"
...
def test_cody_prompt_content_and_version():
    ...
    assert "Ești Cody" in roleplay_prompt
    assert "Codruț" not in roleplay_prompt
