              ...
              return (
                <tr key={recipient.id} className={cn("group/contact grid grid-cols-[auto_minmax(0,1fr)_auto] gap-x-3 gap-y-4 px-4 py-4 transition-colors hover:bg-surface-muted/70 md:table-row md:px-0 md:py-0", !isActive ? "bg-surface-muted/35 text-foreground/55" : null)}>
                  ...
                  <td className={cn("col-start-3 row-start-1 md:table-cell md:px-4 md:py-2.5 md:align-middle", isEditing && "md:align-top")}><div className="flex justify-end gap-1 opacity-80 transition group-hover/contact:opacity-100 group-focus-within/contact:opacity-100 md:gap-2">
                    {isEditing ? <><IconButton label={props.savingContactId === recipient.id ? "Salvăm contactul" : `Salvează ${recipient.email}`} tone="success" disabled={props.savingContactId === recipient.id} onClick={() => props.saveContact(recipient)}><CheckIcon aria-hidden="true" className="size-4" strokeWidth={1.8} /></IconButton><IconButton label={`Anulează editarea pentru ${recipient.email}`} disabled={props.savingContactId === recipient.id} onClick={() => props.cancelEditing(recipient.id)}><XIcon aria-hidden="true" className="size-4" strokeWidth={1.8} /></IconButton></> : <><IconButton label={`Editează ${recipient.email}`} onClick={() => props.startEditing(recipient)}><PencilIcon aria-hidden="true" className="size-4" strokeWidth={1.8} /></IconButton><IconButton label={props.deletingContactId === recipient.id ? "Arhivăm contactul" : `Arhivează ${recipient.email}`} tone="danger" disabled={props.deletingContactId === recipient.id} onClick={() => props.deleteContact(recipient)}><ArchiveIcon aria-hidden="true" className="size-4" strokeWidth={1.8} /></IconButton></>}
                  </div></td>
                </tr>
              );
            }) : (
