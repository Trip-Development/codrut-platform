              ...
              <Card key={team.id} asChild className="gap-0 px-5 [--card-spacing:--spacing(5)]">
                <article>
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
                    ...
                    {team.teamType === "leadership" || team.id === "leadership" ? (
                    ...
                    {team.lencioniAverages.length > 0 ? (
                      team.lencioniAverages.map((item) => (
                        <div key={item.id}>
                          <div className="flex items-baseline justify-between gap-3 text-sm">
                            ...
                            <span className="font-mono font-semibold tabular-nums text-foreground">{item.avg}{scale.suffix}</span>
                          </div>
                          <ScaledBar value={item.avg} min={scale.min} max={scale.max} />
                          {item.interpretation ? (
                      ...
                      <p className="text-sm text-muted-foreground">
                        {reportScaleEmptyCopy(
                          team.lencioniScale,
                          "Nu există încă rezultate scorate pentru echipă.",
                        )}
                      </p>
                    )}
                  </div>
